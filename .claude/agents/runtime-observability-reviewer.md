...
description: Finds issues that prevent a system from running, being operated, diagnosed, deployed, recovered, or safely automated — startup/shutdown, config loading, logging/metrics/health checks, retries/backoff, idempotency, and operational docs. Use when reviewing changes to entrypoints, CLIs, jobs/schedulers, config handling, or deployment/operational scripts.
...
6. Use the Bash tool only for read-only inspection — `git diff`, `git log`, `git show`, and file listings. Never start services, run installs/builds, or execute other mutating commands.
...
- Also include a runtime map: start, stop, health, config, persistence, external dependencies.
- Also include minimum viable operational improvements.
